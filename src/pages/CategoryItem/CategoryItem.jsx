  ...
  useEffect(() => {
    if (currentUser.currentUser) {
      if (currentUser.currentUser.category) {
        const result = currentUser.currentUser.category.filter(
          (e) => e._id === id
        );
        if (result.length === 0) {
          setIsFollow(false);
  ...
  const onSubmit = useCallback(
    (e) => {
      try {
        e.preventDefault();
        dispatch(
          actions.createCategoryUser.createCategoryUserRequest(cateUpdate)
        );
      } catch (err) {
  ...
  const handleSubmit = useCallback(
    (e) => {
      try {
        e.preventDefault();
        dispatch(
          actions.deleteCategoryUser.deleteCategoryUserRequest(cateUpdate)
        );
      } catch (err) {
